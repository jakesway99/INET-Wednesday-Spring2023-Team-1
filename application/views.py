...
def profile_edit(request):
    ...
    context = {'OAuth': token, 'song_form': SongEdit(), 'artist_form': ArtistEdit(), 'album_form': AlbumEdit()}
    ...
    if request.method == 'POST':
        if "song1_id" in request.POST:
            ...
            if FavoriteAlbum.objects.filter(pk=curr_user.pk):  # check if favorite song object exists for user
            ...
            if FavoriteArtist.objects.filter(pk=curr_user.pk):  # check if favorite song object exists for user
